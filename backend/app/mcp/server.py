...
def make_server(engine: Engine | None = None) -> FastMCP:
    ...
    @server.tool(description="全文检索知识库（FTS5）。返回命中文档、分数与行号定位的片段。")
    def search_documents(query: str, limit: int = 20, offset: int = 0) -> dict:
        return tools.search_documents_tool(provider(), query, limit=limit, offset=offset)
